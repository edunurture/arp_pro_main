...
const AcademicReports = () => {
  ...
  const rows = useMemo(
    () => [
      { id: 1, reportId: 'AR001', reportName: 'Attendance Summary Report' },
      { id: 2, reportId: 'AR002', reportName: 'Timetable Utilization Report' },
      { id: 3, reportId: 'AR003', reportName: 'LMS Activity Report' },
      { id: 4, reportId: 'AR004', reportName: 'Examination Performance Report' },
      { id: 5, reportId: 'AR005', reportName: 'Placement Outcome Report' },
    ],
    [],
  )
  ...
  const onActionView = () => {
    ...
    // Report preview page
    window.open(`/reports/academic/${selectedRow.reportId}`, '_blank')
  }

  const onActionPrint = () => {
    if (!selectedRow) return
    // Print-friendly preview page (hook print behavior in preview page)
    window.open(`/reports/academic/${selectedRow.reportId}?print=1`, '_blank')
  }

  const onActionDownload = () => {
    if (!selectedRow) return
    // Static file path (replace with your actual static location)
    window.open(`/static/academic-reports/${selectedRow.reportId}.pdf`, '_blank')
  }
