...
const MiscellaneousReports = () => {
  ...
  const rows = useMemo(
    () => [
      { id: 1, reportId: 'MR001', reportName: 'General Institutional Report' },
      { id: 2, reportId: 'MR002', reportName: 'Administrative Summary Report' },
      { id: 3, reportId: 'MR003', reportName: 'Infrastructure Utilization Report' },
      { id: 4, reportId: 'MR004', reportName: 'Student Support Services Report' },
      { id: 5, reportId: 'MR005', reportName: 'Event & Activities Summary Report' },
    ],
    [],
  )
  ...
  const onActionView = () => {
    ...
    // Report preview page
    window.open(`/reports/miscellaneous/${selectedRow.reportId}`, '_blank')
  }

  const onActionPrint = () => {
    if (!selectedRow) return
    // Print-friendly preview page (hook print behavior in preview page)
    window.open(`/reports/miscellaneous/${selectedRow.reportId}?print=1`, '_blank')
  }

  const onActionDownload = () => {
    if (!selectedRow) return
    // Static file path (replace with your actual static location)
    window.open(`/static/miscellaneous-reports/${selectedRow.reportId}.pdf`, '_blank')
  }
      ...
      <CCol xs={12}>
        ...
        <CCard className="mb-3">
          <CCardHeader className="d-flex justify-content-between align-items-center">
            <strong>MISCELLANEOUS REPORTS</strong>
                    ...
                    pageRows.map((r) => (
                      <CTableRow key={r.id} active={r.id === selectedId}>
                        <CTableDataCell>
                          <CFormCheck
                            type="radio"
                            name="miscellaneousReportsSelect"
